...
using CrewmateOrImpostor.Roles.Impostor;
using CrewmateOrImpostor.Roles.AddOns.Common;
using CrewmateOrImpostor.Roles.AddOns.Impostor;
    ...
    public static class Utils
    {
        ...
        public static void ApplySuffix()
        {
            ...
            else
            {
                if (AmongUsClient.Instance.IsGamePublic)
                    name = $"<color={Main.ModColor}>CrewmateOrImpostor v{Main.PluginVersion}</color>\r\n" + name;
                switch (Options.GetSuffixMode())
                {
                    case SuffixModes.None:
                        break;
                    case SuffixModes.CrewmateOrImpostor:
                        name += $"\r\n<color={Main.ModColor}>CrewmateOrImpostor v{Main.PluginVersion}</color>";
                        break;
        ...
        public static void NotifyRoles(bool isForMeeting = false, PlayerControl SpecifySeer = null, bool NoCache = false, bool ForceLoop = false)
        {
            ...
            foreach (var seer in seerList)
            {
                ...
                var sender = CustomRpcSender.Create("NotifyRoles", Hazel.SendOption.Reliable);
        ...
        {
            var folder = Directory.CreateDirectory($"{Application.persistentDataPath}/CrewmateOrImpostor/Logs");
            if (auto)
            {
        ...
        public static string CopyLog(string path)
        {
            ...
            string fileName = $"{path}/CrewmateOrImpostor-v{Main.PluginVersion}-{t}.log";
