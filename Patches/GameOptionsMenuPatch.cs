...
using HarmonyLib;
using CrewmateOrImpostor.Modules.OptionItems;
using CrewmateOrImpostor.Modules.OptionItems.Interfaces;
using UnityEngine;
...
namespace CrewmateOrImpostor
{
    ...
    {
        private static GameOptionsMenu CrewmateOrImpostorSettingsTab;
        private static PassiveButton CrewmateOrImpostorSettingsButton;
        ...
        {
            CrewmateOrImpostorSettingsTab = Object.Instantiate(__instance.GameSettingsTab, __instance.GameSettingsTab.transform.parent);
            CrewmateOrImpostorSettingsTab.name = CrewmateOrImpostorMenuName;
            var vanillaOptions = CrewmateOrImpostorSettingsTab.GetComponentsInChildren<OptionBehaviour>();
            foreach (var vanillaOption in vanillaOptions)
            {
                Object.Destroy(vanillaOption.gameObject);
            }

            // CrewmateOrImpostor設定ボタンのスペースを作るため，左側の要素を上に詰める
            ...
            // CrewmateOrImpostor設定ボタン
            CrewmateOrImpostorSettingsButton = Object.Instantiate(__instance.GameSettingsButton, __instance.GameSettingsButton.transform.parent);
            CrewmateOrImpostorSettingsButton.name = "CrewmateOrImpostorSettingsButton";
            CrewmateOrImpostorSettingsButton.transform.localPosition = __instance.RoleSettingsButton.transform.localPosition + (__instance.RoleSettingsButton.transform.localPosition - __instance.GameSettingsButton.transform.localPosition);
            CrewmateOrImpostorSettingsButton.buttonText.DestroyTranslator();
            CrewmateOrImpostorSettingsButton.buttonText.text = GetString("CrewmateOrImpostorSettingsButtonLabel");
            var activeSprite = CrewmateOrImpostorSettingsButton.activeSprites.GetComponent<SpriteRenderer>();
            var selectedSprite = CrewmateOrImpostorSettingsButton.selectedSprites.GetComponent<SpriteRenderer>();
            activeSprite.color = selectedSprite.color = Main.UnityModColor;
            CrewmateOrImpostorSettingsButton.OnClick.AddListener((Action)(() =>
            {
                __instance.ChangeTab(-1, false);  // バニラタブを閉じる
                CrewmateOrImpostorSettingsTab.gameObject.SetActive(true);
                __instance.MenuDescriptionText.text = GetString("CrewmateOrImpostorSettingsDescription");
                CrewmateOrImpostorSettingsButton.SelectButton(true);
            }));

            // 各カテゴリの見出しを作成
            MainCategoryHeader = CreateCategoryHeader(__instance, CrewmateOrImpostorSettingsTab, "TabGroup.MainSettings");
            ImpostorRoleCategoryHeader = CreateCategoryHeader(__instance, CrewmateOrImpostorSettingsTab, "TabGroup.ImpostorRoles");
            CrewmateRoleCategoryHeader = CreateCategoryHeader(__instance, CrewmateOrImpostorSettingsTab, "TabGroup.CrewmateRoles");
            NeutralRoleCategoryHeader = CreateCategoryHeader(__instance, CrewmateOrImpostorSettingsTab, "TabGroup.NeutralRoles");
            AddOnCategoryHeader = CreateCategoryHeader(__instance, CrewmateOrImpostorSettingsTab, "TabGroup.Addons");
            ...
            foreach (var option in OptionItem.AllOptions)
            {
                ...
                {
                    var stringOption = Object.Instantiate(template, CrewmateOrImpostorSettingsTab.settingsContainer);
            ...
            CrewmateOrImpostorSettingsTab.Children = scOptions;
            CrewmateOrImpostorSettingsTab.gameObject.SetActive(false);

            // 各カテゴリまでスクロールするボタンを作成
            var jumpButtonY = -0.6f;
            var jumpToMainButton = CreateJumpToCategoryButton(__instance, CrewmateOrImpostorSettingsTab, "CrewmateOrImpostor.Resources.TabIcon_MainSettings.png", ref jumpButtonY, MainCategoryHeader);
            var jumpToImpButton = CreateJumpToCategoryButton(__instance, CrewmateOrImpostorSettingsTab, "CrewmateOrImpostor.Resources.TabIcon_ImpostorRoles.png", ref jumpButtonY, ImpostorRoleCategoryHeader);
            var jumpToCrewButton = CreateJumpToCategoryButton(__instance, CrewmateOrImpostorSettingsTab, "CrewmateOrImpostor.Resources.TabIcon_CrewmateRoles.png", ref jumpButtonY, CrewmateRoleCategoryHeader);
            var jumpToNeutralButton = CreateJumpToCategoryButton(__instance, CrewmateOrImpostorSettingsTab, "CrewmateOrImpostor.Resources.TabIcon_NeutralRoles.png", ref jumpButtonY, NeutralRoleCategoryHeader);
            var jumpToAddOnButton = CreateJumpToCategoryButton(__instance, CrewmateOrImpostorSettingsTab, "CrewmateOrImpostor.Resources.TabIcon_Addons.png", ref jumpButtonY, AddOnCategoryHeader);
        }
        private static MapSelectButton CreateJumpToCategoryButton(GameSettingMenu __instance, GameOptionsMenu CrewmateOrImpostorTab, string resourcePath, ref float localY, CategoryHeaderMasked jumpTo)
        {
            var image = Utils.LoadSprite(resourcePath, 100f);
            var button = Object.Instantiate(__instance.GameSettingsTab.MapPicker.MapButtonOrigin, Vector3.zero, Quaternion.identity, CrewmateOrImpostorTab.transform);
            button.SetImage(image, GameOptionsMenu.MASK_LAYER);
            button.transform.localPosition = new(7.1f, localY, -10f);
            button.Button.ClickMask = CrewmateOrImpostorTab.ButtonClickMask;
            button.Button.OnClick.AddListener((Action)(() =>
            {
                CrewmateOrImpostorTab.scrollBar.velocity = Vector2.zero;  // ドラッグの慣性によるスクロールを止める
                var relativePosition = CrewmateOrImpostorTab.scrollBar.transform.InverseTransformPoint(jumpTo.transform.position);  // Scrollerのローカル空間における座標に変換
                var scrollAmount = CategoryJumpY - relativePosition.y;
                CrewmateOrImpostorTab.scrollBar.Inner.localPosition = CrewmateOrImpostorTab.scrollBar.Inner.localPosition + Vector3.up * scrollAmount;  // 強制スクロール
                CrewmateOrImpostorTab.scrollBar.ScrollRelative(Vector2.zero);  // スクロール範囲内に収め，スクロールバーを更新する
            }));
        ...
        private static CategoryHeaderMasked CreateCategoryHeader(GameSettingMenu __instance, GameOptionsMenu CrewmateOrImpostorTab, string translationKey)
        {
            var categoryHeader = Object.Instantiate(__instance.GameSettingsTab.categoryHeaderOrigin, Vector3.zero, Quaternion.identity, CrewmateOrImpostorTab.settingsContainer);
        ...
        public static void ChangeTabPrefix(bool previewOnly)
        {
            if (!previewOnly)
            {
                if (CrewmateOrImpostorSettingsTab)
                {
                    CrewmateOrImpostorSettingsTab.gameObject.SetActive(false);
                }
                if (CrewmateOrImpostorSettingsButton)
                {
                    CrewmateOrImpostorSettingsButton.SelectButton(false);
                }
            }
        }

        public const string CrewmateOrImpostorMenuName = "CrewmateOrImpostorTab";
    }
    ...
    public class GameOptionsMenuUpdatePatch
    {
        ...
        public static void Postfix(GameOptionsMenu __instance)
        {
            if (__instance.name != GameSettingMenuPatch.CrewmateOrImpostorMenuName) return;
